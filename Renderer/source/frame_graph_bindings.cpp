...
namespace FG
{
	...
	static void CreateDescriptorSet( const FG::FrameGraph* frameGraph, const GpuInputData* inputData, const TechniqueDescriptorSetDesc* descriptorSetDesc, VkDescriptorSetLayout descriptorSetLayout, VkDescriptorPool descriptorPool, VkDescriptorSet* o_descriptorSet )
	{
	...
	static void CreateTechnique( FG::FrameGraph* frameGraph, VkDescriptorPool descriptorPool, const RenderPass* renderpass, const FG::RenderPassCreationData* passCreationData, Technique* o_technique, std::array< GpuInputData, SIMULTANEOUS_FRAMES>& inputBuffers )
	{
		...
		if( passSet )
		{
			...
			for( size_t i = 0; i < SIMULTANEOUS_FRAMES; ++i )
				CreateDescriptorSet( frameGraph, &inputBuffers[i], passSet, o_technique->renderpass_descriptor_layout, descriptorPool, &o_technique->renderPass_descriptor[i] );
		}
		if( instanceSet )
		{
			...
			//TODO: not all of them need one for each simultaneous frames
			for( size_t i = 0; i < SIMULTANEOUS_FRAMES; ++i )
				CreateDescriptorSet( frameGraph, &inputBuffers[i], instanceSet, o_technique->instance_descriptor_layout, descriptorPool, &o_technique->instance_descriptor[i] );
		}
