...
void Load3DTexture( const char* filename, R_HW::GfxImage* o_image, I_ImageAlloctor* allocator )
{
	...
	R_HW::GfxFormat format = R_HW::GfxFormat::R8G8B8A8_UNORM;
	...
	*o_image = CreateCubeImage( texWidth, texHeight, 1, format, R_HW::GfxImageUsageFlagBits::TRANSFER_SRC | R_HW::GfxImageUsageFlagBits::TRANSFER_DST | R_HW::GfxImageUsageFlagBits::SAMPLED );

	R_HW::MarkGfxObject( o_image->image, filename );
	...
	R_HW::MarkGfxObject( o_image->imageView, filename );
}
...
{
	*o_image = R_HW::CreateImage( width, height, miplevels, format, R_HW::GfxImageUsageFlagBits::TRANSFER_SRC | R_HW::GfxImageUsageFlagBits::TRANSFER_DST | R_HW::GfxImageUsageFlagBits::SAMPLED );
...
void Load2DTextureFromFile( const char* filename, R_HW::GfxImage* o_image, I_ImageAlloctor* allocator )
{
	...
	R_HW::GfxDeviceSize imageSize = texWidth * texHeight * 4;
	const uint32_t mipLevels = 1; /*static_cast< uint32_t >(std::floor( std::log2( std::max( texWidth, texHeight ) ) )) + 1;*/

	if( !pixels )
		throw std::runtime_error( "failed to load texture image!" );

	*o_image = R_HW::CreateImage( ( uint32_t )texWidth, ( uint32_t )texHeight, mipLevels, R_HW::GfxFormat::R8G8B8A8_UNORM,
		R_HW::GfxImageUsageFlagBits::TRANSFER_SRC | R_HW::GfxImageUsageFlagBits::TRANSFER_DST | R_HW::GfxImageUsageFlagBits::SAMPLED );
	R_HW::MarkGfxObject( o_image->image, filename );
...
void CreateSolidColorImage( glm::vec4 color, R_HW::GfxImage* o_image, I_ImageAlloctor* allocator )
{
	...
	const R_HW::GfxDeviceSize memorySize = width * height * 4;
	...
	*o_image = R_HW::CreateImage( width, height, 1, R_HW::GfxFormat::R8G8B8A8_UNORM, R_HW::GfxImageUsageFlagBits::TRANSFER_SRC | R_HW::GfxImageUsageFlagBits::TRANSFER_DST | R_HW::GfxImageUsageFlagBits::SAMPLED );
...
void generateMipmaps( R_HW::GfxCommandBuffer commandBuffer, R_HW::GfxApiImage image, R_HW::GfxFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels )
{
	...
	for( uint32_t i = 1; i < mipLevels; ++i ) {
		R_HW::GfxImageBarrier( commandBuffer, image, R_HW::GfxLayout::TRANSFER, R_HW::GfxAccess::WRITE, R_HW::GfxLayout::TRANSFER, R_HW::GfxAccess::READ, i - 1, 1, 0, 1 );
		...
		R_HW::CmdBlitImage( commandBuffer, image, 0, 0, 0, mipWidth, mipHeight, 1, srcMip,
			image, 0, 0, 0, dstMipWidth, dstMipHeight, 1, dstMip, R_HW::GfxFilter::LINEAR );

		R_HW::GfxImageBarrier( commandBuffer, image, R_HW::GfxLayout::TRANSFER, R_HW::GfxAccess::READ, R_HW::GfxLayout::COLOR, R_HW::GfxAccess::READ, i - 1, 1, 0, 1 );
	...
	R_HW::GfxImageBarrier( commandBuffer, image, R_HW::GfxLayout::TRANSFER, R_HW::GfxAccess::WRITE, R_HW::GfxLayout::COLOR, R_HW::GfxAccess::READ, mipLevels - 1, 1, 0, 1 );
}