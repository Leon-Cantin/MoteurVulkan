...
void CreateTimeStampsQueryPool(uint32_t setCount)
{
	...
	vkCreateQueryPool(g_vk.device, &poolCreateInfo, nullptr, &g_timeStampQueryPool);
}
...
{
	vkDestroyQueryPool(g_vk.device, g_timeStampQueryPool, nullptr);
}
...
float GetTimestampsDelta(Timestamp first, Timestamp last, uint32_t set)
{
	...
	if(last - first == 1)
		vkGetQueryPoolResults(g_vk.device, g_timeStampQueryPool, first + set * Timestamp::TIMESTAMP_COUNT, 2, sizeof(values), values, sizeof(uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
	else
	{
		vkGetQueryPoolResults(g_vk.device, g_timeStampQueryPool, first + set * Timestamp::TIMESTAMP_COUNT, 1, sizeof(uint64_t), &values[0], sizeof(uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
		vkGetQueryPoolResults(g_vk.device, g_timeStampQueryPool, last + set * Timestamp::TIMESTAMP_COUNT, 1, sizeof(uint64_t), &values[1], sizeof(uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
	}
